...
# 打开数据库连接                127.0.0.1
Tang = pymysql.connect(host="localhost", port=3306, user="root", password="123456", database="one",
                       charset="utf8")
xiao = Tang.cursor()
...
def reconnect():
    ...
    try:
        db = pymysql.connect(host="localhost", port=3306, user="root", password="123456", database="one",
                             charset="utf8")
        return db
...
def hhhh():
    ...
    # 2.绑定 bind
    server.bind(('192.168.100.145', 9040))
...
def data_addwendu():
    ...
    if request.method == "POST":
        ...
        try:
            cursor.execute("insert into data(wendu,wendu2) values (\"" + str(wendu) +
                           "\",\"" + str(wendu2) + "\")")
            db.commit()  # 提交，使操作生效
            print("add  successfully!")
            return "1"
...
def state_twofan():
    ...
    if request.method == "POST":
        ...
        try:
            cursor.execute("update state set twofan=\"" + str(twofan) + "\"")
            db.commit()  # 提交，使操作生效
            print("update successfully!")
            TwofanChange = True
